...
class LCA:
    def __init__(self):
        ...
        self.w = []           # 新增1：每个结点的零食编号
        self.fw = []          # fw[i][j] 代表从根结点到i结点，有多少个种类为j的零食
    ...
    def Initialize(self, n):
        ...
        # 新增2：初始化每个结点的零食编号
        self.w = [0] * (n + 1)
        self.fw = [[0] * 21 for _ in range(n + 1)]
    ...
    def PreProcess(self, root):
        ...
        while self.q:
            ...
            for v in self.adj[u]:
                ...
                # 新增4：树上前缀和
                for j in range(1, 21):
                    self.fw[v][j] = self.fw[u][j] + (1 if self.w[v] == j else 0)
    def Get(self, u, v):
        # 1、确保 u的深度 >= v的深度
...
lca = LCA()
lca.Initialize(n)
lca.w  = [0] + list( map(int, input().split()))
...
for _ in range(q):
    ...
    for j in range(1, 21):
        val = (lca.fw[x][j] - lca.fw[u][j]) + (lca.fw[y][j] - lca.fw[u][j]) + (1 if lca.w[u] == j else 0)
