...
class DpData:
    ...
    def getNextDpData(self, is_leadingZero, digit):
        ...
        if not is_leadingZero:
            # 0000122313
            if abs(digit - self.data0) > self.K:
                ret.data1 = 0
...
# 初始化dp数组
dp = [[[[[-1 for _ in range(2)] for __ in range(10)] for ___ in range(2)] for ____ in range(2)] for _____ in range(maxd)]
